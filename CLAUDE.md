...
- **Route composition, not a router file.** Each `src/routes/<domain>/index.ts` builds a Hono instance and is mounted at `/api/<domain>` via `app.route()` (see `routes/library/index.ts` — list/meta/grab/files/job split). `src/index.ts` only `.route()`s the domain routers; order matters, because rate-limit and auth middleware are wired in between them. The API moved off Elysia onto Hono (PR #119, 2026-09-10) — see `docs/superpowers/archive/plans/2026-09-10-api-hono-migration.md` for the migration map.
