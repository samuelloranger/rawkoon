...
// 3. Find our build (by CFBundleVersion), waiting for it to register after upload
let build = null;
for (let attempt = 1; attempt <= 30 && !build; attempt++) {
  const builds = await api(`/v1/builds?filter[app]=${app.id}&filter[version]=${encodeURIComponent(BUILD_NUMBER)}&limit=1`);
  build = builds.json.data?.[0] || null;
  if (!build) {
    console.log(`build #${BUILD_NUMBER} not visible yet (attempt ${attempt}), waiting…`);
    await sleep(20_000);
  }
}
if (!build) { console.error(`Build #${BUILD_NUMBER} never appeared`); process.exit(1); }
console.log(`build ${build.id} #${build.attributes?.version} state=${build.attributes?.processingState}`);
...
console.log(`attached build #${BUILD_NUMBER} to "${internal.attributes?.name}" (status ${attach.status})`);
