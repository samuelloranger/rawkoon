...
/**
 * Releases shown to the judge. Prompt processing measured ~160 tok/s on the
 * homelab Arc A380, and a real interactive search returned 69 releases —
 * 19,706 tokens against a 16,384 context, which the server rejects outright.
 * The prompt ranks on score anyway, so the tail can only spend budget.
 */
 ...
 * prompt is a mechanical filter, and a 4B model reliably violated it when the
